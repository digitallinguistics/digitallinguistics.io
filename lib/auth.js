      ...
      if (req.cookies.register) { res.redirect('/register?' + qs.stringify({ state: req.query.state })); }
        ...
        .then(result => {
          if (result.status === 404) {
            res.render('error', { status: 404, details: `No DLx user found with that login. <a href=/login>Try logging in with a different service</a>, or <a href=/login?register=true>create a new account</a>.` });
          } else {
  ...
  if (!authReq) { res.render('error', { status: 500, details: 'There was a problem registering. Please try your request again.' }); }
    ...
    .then(result => {
      if (result.status !== 201) {
        res.render('error', { status: result.status, details: result.error + ' ' + result.error_description });
      } else {
        res.login(result.data.rid);
