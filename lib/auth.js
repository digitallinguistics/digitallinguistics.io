...
exports.login = (req, res) => {
  if (req.authStatus !== 'valid') {
    res.status(401).json({ status: 400, error: 'Bad request. The request URL, headers, or body are invalid.', error_description: 'Please provide the DLx `secret` parameter in the querystring.' });
  } else if (!req.query.redirect) {
    res.status(400).json({ status: 400, error: 'Bad request. The request URL, headers, or body are invalid.', error_description: 'Please provide a redirect parameter in the querystring.' });
  } else {
  ...
  if (!authReq) { res.render('error', { status: 500, details: 'There was a problem authenticating. Please try your request again.' }); }
    ...
    .then(serviceId => {
      ...
      if (req.cookies.register) { res.redirect('/register?' + qs.stringify({ state: req.query.state })); }
        ...
        api.getUserByServiceId(serviceId)
        .then(user => {
          res.login(user.rid);
          res.redirect(authReq.redirect + '?' + qs.stringify({ state: req.query.state }));
        }).catch(err => res.render('error', { status: 500, deatils: err }));
      }
