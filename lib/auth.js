...
exports.oauth = (req, res) => {
  ...
  if (!authReq) { res.render('error', { status: 500, details: 'There was a problem authenticating the user. Please try your request again.' }); }
  else {
    ...
    const register = () => new Promise(resolve => {
      if (req.cookies.register) {
        ...
        res.redirect('/register?redirect=' + authReq.redirect);
      } else {
        resolve();
      }
    });
